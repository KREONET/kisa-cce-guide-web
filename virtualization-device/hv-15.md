...
targetIdentifiers:
  - linux
sourceTargetText: "VMware ESXi, vCenter, KVM"

provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 833
      physicalPageEnd: 835
      printedPageStart: "833"
      printedPageEnd: "835"
...
4. 로그 기록 정책 확인 후, [옵션 편집]을 클릭해 내부 정책에 맞게 설정
...
2. vSphere Client 접속 후, 다음 메뉴에 접근하여 확인(vSphere Client 버전에 따라, 메뉴 명칭은 달라질 수 있음)
...
2. libvirt 설정파일을 확인하여 로그 레벨 확인

   ```shell command
   $ cat /etc/libvirt/libvirtd.conf
   ```

3. libvirt 설정파일의 `log_level` 설정 구문 수정

   ```text configuration
   $ log_level =
   ```

4. 변경 사항 적용을 위해 libvirt 데몬 재시작
