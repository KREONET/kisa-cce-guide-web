...
5. `username secret` 설정
   ...
   `(config)# username secret` 명령어를 사용하여 로컬 사용자 비밀번호를 일방향 암호화 저장. `enable secret`과 `enable password` 명령어로 비밀번호를 설정하여 같이 사용하는 경우 `enable secret` 명령어로 설정한 비밀번호의 우선순위가 높으며 보안상 비밀번호 서로 다르게 입력해야 함
