...
targetIdentifiers:
  - linux
sourceTargetText: "Oracle DB, MSSQL, MySQL, Tibero, PostgreSQL 등"
...
3. mysql 5.7 비밀번호 및 암호화 알고리즘 설정, user 생성 시 적용

   ```sql command
   CREATE USER '계정명'@'host' IDENTIFIED BY '비밀번호';
   ```

4. mysql 5.7 비밀번호 및 암호화 알고리즘 설정, 기존 user 적용
...
5. mysql 8.0 비밀번호 및 암호화 알고리즘 설정, user 생성 시 적용

   ```sql command
   mysql> CREATE USER '계정명'@'localhost' IDENTIFIED WITH caching_sha2_password BY '비밀번호';
   ```

6. mysql 8.0 비밀번호 및 암호화 알고리즘 설정, 기존 user 적용
