...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 633
      physicalPageEnd: 634
      printedPageStart: "633"
      printedPageEnd: "634"
sourceAnnotations:
  - annotationIdentifier: d-11-source-001
    annotationType: conversionDecision
    targetType: astNode
    targetReference: "d-11:remediation.supplementaryGuidance.command:1"
    sourceLocation:
      physicalPage: 633
      printedPage: "633"
      pageRegionIdentifier: p633-d-11
    sourceText: "AQ_AD"
    explanation: "원문의 철자와 지면 줄바꿈을 보존했다. 줄바꿈으로 분리된 토큰의 결합 의도는 확인이 필요하다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 633의 SQL 문을 이미지에서 직접 대조했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
...
1. system tables 접근 권한이 PUBLIC, GUEST 또는 비인가된 사용자에게 부여된 경우 접근 권한을 제거
...
2. 접근이 필요한 데이터베이스 및 테이블에만 권한 적용
...
1. sys_tables_를 조회하여 system_ 외 접근 계정 유무 확인

   ```sql command
   SELECT * FROM system_.sys_tables_;
   ```

2. 불필요 계정 접근 시 해당 접근 해제
...
2. 스키마명에 해당되는 Table에 대한 접근 권한을 일반 사용자로부터 제거
