...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 646
      physicalPageEnd: 647
      printedPageStart: '646'
      printedPageEnd: '647'
sourceAnnotations:
  - annotationIdentifier: d-17-inventory-001
    ...
    sourceLocation:
      physicalPage: 646
      printedPage: '646'
      pageRegionIdentifier: p646-d-17
    sourceText: Audit Table은 데이터베이스 관리자 계정으로 접근하도록 제한
    explanation: '{"anomalyType": "detailHeaderTitleMismatch", "code": "D-17", "detailHeaderTitle": "Audit Table은 데이터베이스 관리자 계정으로 접근하도록 제한", "physicalPage": 646, "resolution": "Use the detailed criterion heading as the canonical title and preserve the chapter summary value as an annotation.", "summaryTitle": "Audit Table은 데이터베이스 관리자 계정에 접근하도록 제한"}'
...
~~~text transcription
개요
점검 내용 Audit Table 접근 권한이 관리자 계정으로 제한되고 있는지 점검
...
보안 위협 등을 수행할 수 있으므로 보안 사고 발생 시 원인 분석이 불가능하게 되며, 이로 인해 재발 방지를 위한
...
조치 방법 Audit Table 접근 권한을 관리자 계정으로 제한
...
![PDF 페이지 646의 원문 점검항목 영역](../assets/d-17/d-17-page-646-source-region.png)
...
![PDF 페이지 647의 원문 점검항목 영역](../assets/d-17/d-17-page-647-source-region.png)
