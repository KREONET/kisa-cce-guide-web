...
2. Audit table에 접근할 권한이 없는 계정이 확인될 경우 권한 삭제(SYS 또는 SYSTEM 제외)
...
1. 사용자 계정을 조회하여 `SYSTEM_`, `SYS`의 `USER_ID`를 확인

   ```sql command
   SELECT * FROM system_.sys_users_;
   ```

2. 시스템 테이블 조회 내용 중 `AUDIT` 관련 테이블 정보의 `TABLE_ID` 확인

   ```sql command
   SELECT * FROM system_.sys_tables_;
   ```

3. Audit Table의 계정에 불필요한 권한이 부여되어 있는 경우 권한 삭제
