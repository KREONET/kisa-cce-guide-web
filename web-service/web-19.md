...
targetIdentifiers:
  ...
  - aix
  - hp-ux
sourceTargetText: "Apache, Tomcat, Nginx, IIS, WebtoB"

provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 325
      physicalPageEnd: 328
      printedPageStart: "325"
      printedPageEnd: "328"

sourceAnnotations:
  - annotationIdentifier: web-19-source-001
    annotationType: sourceInconsistency
    targetType: astNode
    targetReference: "web-19:remediation.linux.command:3"
    sourceLocation:
      physicalPage: 326
      printedPage: "326"
      pageRegionIdentifier: p326-web-19
    sourceText: "#cat /[Tomcat 설치 디렉터리]/tomcat-users.xml | grep ‘SSIServlet\\|SSIFilter’"
    explanation: "절차 설명은 web.xml을 지칭하지만, 확인 명령은 tomcat-users.xml을 지칭한다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 326의 Tomcat Step 1 설명과 바로 다음 명령을 대조했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
...
웹 서비스 내 SSI 사용을 제한하지 않을 경우, 공격자가 SSI 기능을 이용하여 시스템 명령 실행 및 중요 파일 탈취 등 공격이 가능하며, 이를 통해 서버 시스템 침해, 데이터 유출 등이 발생할 위험이 존재하며, SSI 공격 시 HTML 페이지에 스크립트를 삽입하거나 원격으로 코드를 실행하여 웹 서비스를 악용할 위험이 존재함
...
> **참고**
>
> **SSI(Server-Side Includes) Injection:** HTML 문서 내 입력받은 변수값을 서버 측에서 처리할 때 부적절한 명령문이 포함 및 실행되어 서버의 데이터가 유출되는 취약점
