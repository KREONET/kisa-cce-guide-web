...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 308
      physicalPageEnd: 310
      printedPageStart: "308"
      printedPageEnd: "310"

sourceAnnotations:
  - annotationIdentifier: web-13-source-001
    annotationType: sourceInconsistency
    targetType: astNode
    targetReference: "web-13:remediation.supplementaryGuidance.note.content:1"
    sourceLocation:
      physicalPage: 309
      printedPage: "309"
      pageRegionIdentifier: p309-web-13
    sourceText: "asa/asax 스크립트 매핑 또는 파일 필터링 중 하나라도 설정 시 취약"
    explanation: "처리기 매핑 제거와 파일 필터링 등록을 조치로 제시한 앞선 절차와 달리, 둘 중 하나라도 설정하면 취약하다고 기재되어 의미가 상충한다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 309의 IIS Step 1, Step 2 및 바로 다음 참고 문구를 대조했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
...
2. DB 연결 리소스가 존재하는 설정 파일 접근권한을 `600`으로 설정
...
2. 요청 필터링에서 DB 매핑 설정제한
   ...
   `“허용됨”` 값이 `true`인 매핑 제거 및 `“파일 이름 확장명 거부”`에 등록
