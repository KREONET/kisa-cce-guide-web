...
1. **Apache:** `apache2.conf` (또는 `/conf/httpd.conf`) 파일 내 `DocumentRoot`를 별도의 경로로 변경

   ```text configuration
   DocumentRoot [별도의 경로]
   ```

2. **Tomcat:** `web.xml` 파일 내 `docBase`를 별도의 경로로 변경
...
3. **Nginx:** `sites-available` 파일 내 `DocumentRoot`를 별도의 경로로 변경
...
5. **JEUS:** `ws_engine.m` 파일 내 `Docroot`을 별도의 경로로 변경

   ```text configuration
   Docroot = “[별도의 경로]”
   ```

6. **WebtoB:** `http.m` 파일 내 `DOCROOT`를 별도의 경로로 변경
