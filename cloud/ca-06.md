...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 859
      physicalPageEnd: 859
      printedPageStart: '859'
      printedPageEnd: '859'
sourceAnnotations: []
---
...
~~~text transcription
개요
점검 내용 가상 네트워크 서비스 리소스 생성 또는 접근 권한 설정 여부 점검
점검 목적 가상 네트워크 서비스 리소스 생성 또는 접근 권한을 점검하여 서비스 내부 정보 유출을 방지하기 위함
네트워크 서비스 접근 권한이 적절하지 않으면 비인가자가 서비스 내부에 침입하여 중요 정보를 유출하
보안 위협
는 위험이 존재함
참고 ※ 네트워크 서비스: 클라우드 서비스에서 제공하는 네트워크 리소스
점검 대상 및 판단 기준
대상 클라우드 플랫폼
양호 : 네트워크 서비스 리소스 접근 권한이 적절하게 부여된 경우
판단 기준
취약 : 네트워크 서비스 리소스 접근 권한이 적절하게 부여되지 않은 경우
조치 방법 네트워크 서비스 리소스 생성 및 접근 권한을 관리자 계정에 별도로 부여
...
![PDF 페이지 859의 원문 점검항목 영역](../assets/ca-06/ca-06-page-859-source-region.png)
