...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 276
      physicalPageEnd: 277
      printedPageStart: "276"
      printedPageEnd: "277"

sourceAnnotations:
  - annotationIdentifier: web-02-source-001
    annotationType: sourceInconsistency
    targetType: astNode
    targetReference: "web-02:remediation.supplementaryGuidance.step:10"
    sourceLocation:
      physicalPage: 277
      printedPage: "277"
      pageRegionIdentifier: p277-web-02
    sourceText: "연속적인 문자나 숫자 사용"
    explanation: "항목 자체에는 사용 금지라는 표현이 없지만, 바로 다음 예시는 1111, 1234, abcd 사용금지로 기재되어 있다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 277의 비밀번호 설정 기준 2.(4)와 바로 다음 예시를 대조했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
  - annotationIdentifier: web-02-source-002
    annotationType: sourceInconsistency
    targetType: astNode
    targetReference: "web-02:remediation.supplementaryGuidance.step:13"
    sourceLocation:
      physicalPage: 277
      printedPage: "277"
      pageRegionIdentifier: p277-web-02
    sourceText: "SAM 파일에 암호를 저장하기 위해 사용되는 LANMan 알고리즘은"
    explanation: "비밀번호 설정 기준의 2번 항목 다음에 번호 1이 다시 나타나고, 그 다음 항목은 3번으로 이어진다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 277의 항목 번호 순서를 확인했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
...
1. **Tomcat — Step 1)** 복잡도를 만족하는 비밀번호 설정
...
2. **Tomcat — Step 2)** Tomcat 재시작

   ```bash command
   # systemctl restart tomcat
   ```

3. **JEUS — Step 1)** `Lock & EDIT` > `Security` > `Security Domains` 페이지 해당 도메인 > `Account & Policies Management` > `Users` > 기본 관리자 계정의 비밀번호 변경 > 확인 > `“Activate Changes”`을 눌러 설정 저장
...
1. SAM 파일에 암호를 저장하기 위해 사용되는 LANMan 알고리즘은 8자 단위로 글자를 나누어 암호화하기 때문에 8의 배수가 되는 암호 사용 권장 (8자로 이루어진 암호 사용 권장)
