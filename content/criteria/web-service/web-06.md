...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 286
      physicalPageEnd: 289
      printedPageStart: "286"
      printedPageEnd: "289"

sourceAnnotations:
  - annotationIdentifier: web-06-source-001
    annotationType: sourceInconsistency
    targetType: astNode
    targetReference: "web-06:remediation.unspecified.step:1"
    sourceLocation:
      physicalPage: 286
      printedPage: "286"
      pageRegionIdentifier: p286-web-06
    sourceText: "Authconfig"
    explanation: "Step 1은 Authconfig로, Step 2는 AuthConfig로 표기되어 대소문자가 일치하지 않는다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 286의 Apache Step 1과 Step 2 제목을 대조했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
...
상위 디렉터리로 이동하는 것이 가능할 경우 접근하고자 하는 디렉터리의 하위 경로에서 상위로 이동하며 정보 탐색이 가능하여 중요 정보가 노출될 위험이 존재하며, 악의적인 목적을 가진 사용자가 중요 파일 및 디렉터리의 접근이 가능하여 데이터가 유출될 위험이 존재함
...
1. `enableParentPaths` 요소를 `False`로 설정

   ```text literal
   제어판 > 관리 도구 > 인터넷 정보 서비스(IIS) 관리자 > 해당 웹사이트 > 사이트 편집에서 루트 디렉터리 > 루트 디렉터리의 web.config 파일
   ```
