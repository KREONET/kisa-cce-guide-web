...
targetIdentifiers:
  ...
  - aix
  - hp-ux
sourceTargetText: "Oracle DB, Altibase, Tibero, Cubrid 등"

provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 648
      physicalPageEnd: 649
...
1. 사용자 정보를 조회하여 Object 권한, 시스템 권한이 Public 또는 Guest에게 부여되어 있는지 확인
   ...
   `GRANTOR_ID`: 권한을 부여한 사용자의 식별자로, `SYS_USERS_` 메타테이블의 한 `USER_ID` 값과 동일. `GRANTEE_ID`: 권한을 부여받은 사용자의 식별자로, `SYS_USERS_` 메타테이블의 한 `USER_ID` 값과 동일함. 단, Object 권한을 Public에게 부여한 경우, `SYS_USERS_` 메타테이블에 존재하지 않는 `USER_ID` 값인 `"0"`이 칼럼에 나타남
...
1. 사용자 정보를 조회하여 Role 부여가 적절한지 확인
