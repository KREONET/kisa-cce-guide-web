...
targetIdentifiers:
  - linux
sourceTargetText: "Oracle DB, Altibase, Tibero, PostgreSQL 등"
...
| Priv ID | Object Privilege | Table | Sequence | PSM/External Procedure | View | Directory | External Library |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 101 | ALTER | ○ | ○ |  |  |  |  |
| 102 | DELETE | ○ |  |  |  |  |  |
| 103 | EXECUTE |  |  | ○ |  |  | ○ |
...
모든 사용자는 자동으로 PUBLIC에 부여되어 SELECT 권한을 가진다.

**Tibero**

1. 데이터베이스 내 모든 스키마 Object 특권의 정보를 조회하여 인가받지 않은 Object 권한 소유자가 있는지 확인

   ```sql command
   SELECT * FROM dba_tbl_privs;
   ```

2. 잘못된 Object 권한 소유자 발견 시 권한 회수
