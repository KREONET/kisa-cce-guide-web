...
> **참고**
>
> **SMTP(Simple Mail Transfer Protocol) 서버:** SMTP에 의해 전자 메일을 발신하는 서버(server)를 SMTP 서버라고 함
...
메일 서비스를 사용하지 않는 경우 서비스 중지 및 비활성화 설정. 메일 서비스 사용 시 릴레이 방지 설정 또는 릴레이 대상 접근 제어 설정
...
4. `/etc/mail/access` 파일에 특정 IP, Domain, Email 주소, 네트워크에 대한 접근제한 설정
   ...
   `/etc/mail/access` 파일을 생성하거나 수정하였을 경우 다음 명령으로 DB 파일 생성
