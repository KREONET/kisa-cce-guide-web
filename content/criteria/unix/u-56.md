---
schemaVersion: 1
contentModel: systemCriterion
contentModelVersion: 1

criterion:
  code: U-56
  slug: u-56
  title: FTP 서비스 접근 제어 설정
  severity:
    level: low
    sourceLabel: 하

classification:
  domainIdentifier: unix
  categoryIdentifier: unix-service-management

targetScope: nonExhaustive
targetIdentifiers:
  - solaris
  - linux
  - aix
  - hp-ux
sourceTargetText: "SOLARIS, LINUX, AIX, HP-UX 등"

provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 135
      physicalPageEnd: 138
      printedPageStart: "135"
      printedPageEnd: "138"

sourceAnnotations:
  - annotationIdentifier: u-56-source-001
    annotationType: sourceTypographicalError
    targetType: astNode
    targetReference: "u-56:remediation.solaris.command:8"
    sourceLocation:
      physicalPage: 136
      printedPage: "136"
      pageRegionIdentifier: p136-u-56
    sourceText: "/etc/vsftpd/ftpuser"
    explanation: "같은 절차의 다른 단계에 표기된 /etc/vsftpd/ftpusers와 달리 끝의 s가 없다. 원문을 그대로 보존했다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 136의 vsFTP – ftpusers Step 2부터 Step 6까지 대조했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
  - annotationIdentifier: u-56-source-002
    annotationType: sourceInconsistency
    targetType: astNode
    targetReference: "u-56:remediation.solaris.step:26"
    sourceLocation:
      physicalPage: 137
      printedPage: "137"
      pageRegionIdentifier: p137-u-56
    sourceText: "파일 소유자 및 권한 확인"
    explanation: "단계 설명은 소유자 및 권한 확인이지만 제시된 명령은 cat을 사용한 파일 내용 출력이다. 원문을 그대로 보존했다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 137의 ProFTP – proftpd.conf Step 2 설명과 명령을 대조했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
---

## 개요

### 점검 내용

FTP 서비스에 비인가자의 접근 가능 여부 점검

### 점검 목적

접근 권한이 없는 비인가자의 접근을 통제하기 위함

### 보안 위협

FTP 서비스의 접근제한 설정이 적절하지 않을 경우, 인증 절차 없이 비인가자가 디렉터리나 파일에 접근할 수 있어 중요 파일 변조 및 유출을 시도할 위험이 존재함

### 참고

> **참고**
>
> 기반시설 시스템에서 FTP 서비스의 이용은 원칙적으로 금지하나, 불가피하게 사용 시 접근 제어 설정 등의 보안 조치를 반드시 적용해야 함

## 점검 대상 및 판단 기준

### 대상

SOLARIS, LINUX, AIX, HP-UX 등

### 판단 기준

- **양호:** 특정 IP주소 또는 호스트에서만 FTP 서버에 접속할 수 있도록 접근 제어 설정을 적용한 경우
- **취약:** FTP 서버에 접근 제어 설정을 적용하지 않은 경우

### 조치 방법

FTP 서비스를 사용하지 않는 경우 서비스 중지 및 비활성화 설정, FTP 서비스 사용 시 접근 제어 설정

### 조치 시 영향

특정 IP주소 또는 호스트에서만 FTP 접속이 가능함

## 점검 및 조치 사례

### SOLARIS

다음 절차는 LINUX, AIX, HP-UX에도 적용함

**FTP – ftpusers**

1. `ftpusers` 파일 소유자 및 권한 확인

   ```shell command
   # ls -l /etc/ftpusers 또는 # ls -l /etc/ftpd/ftpusers
   ```

2. 접근 제한 설정 확인

   ```shell command
   # cat /etc/ftpusers 또는 # cat /etc/ftpd/ftpusers
   ```

3. 파일 소유자를 `root`로 변경

   ```shell command
   # chown root /etc/ftpusers 또는 # chown root /etc/ftpd/ftpusers
   ```

4. 파일 권한을 `640`으로 변경

   ```shell command
   # chmod 640 /etc/ftpusers 또는 # chmod 640 /etc/ftpd/ftpusers
   ```

5. `/etc(/ftpd)/ftpusers` 파일에 FTP 서비스에 접근을 차단할 사용자 설정

**vsFTP – ftpusers**

1. `userlist_enable` 설정값 확인

   ```shell command
   # cat /etc/vsftpd.conf | grep userlist_enable 또는 # cat /etc/vsftpd/vsftpd.conf | grep userlist_enable”
   ```

   ```text output
   userlist_enable = NO
   ```

2. 파일 소유자 및 권한 확인

   ```shell command
   # ls -l /etc/vsftpd.ftpusers 또는 # ls -l /etc/vsftpd/ftpusers
   ```

3. 접근 제한 설정 확인

   ```shell command
   # cat /etc/vsftpd.ftpusers 또는 # cat /etc/vsftpd/ftpusers
   ```

4. 파일 소유자를 `root`로 변경

   ```shell command
   # chown root /etc/vsftpd.ftpusers 또는 # chown root /etc/vsftpd/ftpuser
   ```

5. 파일 권한을 `640`으로 변경

   ```shell command
   # chmod 640 /etc/vsftpd.ftpusers 또는 # chmod 640 /etc/vsftpd/ftpusers
   ```

6. `/etc/vsftpd.ftpusers` 또는 `/etc/vsftpd/ftpusers` 파일에 FTP 서비스에 접근을 차단할 사용자 설정

**vsFTP – user_list**

1. `userlist_enable` 설정값 확인

   ```shell command
   # cat /etc/vsftpd.conf | grep userlist_enable 또는 # cat /etc/vsftpd/vsftpd.conf | grep userlist_enable
   ```

   ```text output
   userlist_enable = YES
   ```

2. 파일 소유자 및 권한 확인

   ```shell command
   # ls -l /etc/vsftpd.user_list 또는 # ls -l /etc/vsftpd/user_list
   ```

3. 접근 제한 설정 확인

   ```shell command
   # cat /etc/vsftpd.user_list 또는 # cat /etc/vsftpd/user_list
   ```

4. 파일 소유자를 `root`로 변경

   ```shell command
   # chown root /etc/vsftpd.user_list 또는 # chown root /etc/vsftpd/user_list
   ```

5. 파일 권한을 `640`으로 변경

   ```shell command
   # chmod 640 /etc/vsftpd.user_list 또는 # chmod 640 /etc/vsftpd/user_list
   ```

6. `/etc(/vsftpd)/vsftpd.conf` 파일의 `userlist_deny` 옵션 설정

   ```text configuration
   user_list에 등록된 사용자만 접속 허가 : userlist_deny=no
   user_list에 등록된 사용자 접속 차단 : userlist_deny=yes
   ```

7. `/etc(/vsftpd)/vsftpd.conf` 파일에 FTP 서비스에 접근을 허가/차단할 사용자 설정

**ProFTP – ftpusers**

1. `UseFtpUsers` 설정 확인

   ```shell command
   # cat /etc/proftpd.conf | grep UseFtpUsers 또는 # cat /etc/proftpd/proftpd.conf | grep UseFtpUsers
   ```

   ```text output
   UseFtpUsers on (기본 설정 : on)
   ```

2. 파일 소유자 및 권한 확인

   ```shell command
   # ls -l /etc/ftpusers 또는 # ls -l /etc/ftpd/ftpusers
   ```

3. 접근 제한 설정 확인

   ```shell command
   # cat /etc/ftpusers 또는 # cat /etc/ftpd/ftpusers
   ```

4. 파일 소유자를 `root`로 변경

   ```shell command
   # chown root /etc/ftpusers 또는 # chown root /etc/ftpd/ftpusers
   ```

5. 파일 권한을 `640`으로 변경

   ```shell command
   # chmod 640 /etc/ftpusers 또는 # chmod 640 /etc/ftpd/ftpusers
   ```

6. `/etc(/ftpd)ftpusers` 파일에 FTP 서비스에 접근을 차단할 사용자 설정

**ProFTP – proftpd.conf**

1. `UseFtpUsers` 설정 확인

   ```shell command
   # cat /etc/proftpd.conf | grep UseFtpUsers 또는 # cat /etc/proftpd/proftpd.conf | grep UseFtpUsers
   ```

   ```text output
   UseFtpUsers off
   ```

2. 파일 소유자 및 권한 확인

   ```shell command
   # cat /etc/proftpd.conf 또는 # cat /etc/proftpd/proftpd.conf
   ```

3. 접근 제한 설정 확인

   ```shell command
   # sed -n ‘/<Limit LOGIN>/, /<\/Limit>/p’ /etc/proftpd.conf 또는 # sed -n ‘/<Limit LOGIN>/, /<\/Limit>/p’
   /etc/proftpd/proftpd.conf”
   ```

4. 파일 소유자를 `root`로 변경

   ```shell command
   # chown root /etc/proftpd.conf 또는 # chown root /etc/proftpd/proftpd.conf
   ```

5. 파일 권한을 `640`으로 변경

   ```shell command
   # chmod 640 /etc/proftpd.conf 또는 # chmod 640 /etc/proftpd/proftpd.conf
   ```

6. `/etc(/proftpd)/proftpd.conf` 파일 수정 및 삽입

   ```apache configuration
   <Limit LOGIN>
   Order Deny,Allow
   AllowUser <사용자 이름> 또는 Allow from <IP주소>
   DenyUser <사용자 이름> 또는 Deny from <IP주소>
   </Limit>
   ```

7. ProFTP 서비스 재시작

> **참고**
>
> `Order`: 먼저 정의된 설정 우선 적용
