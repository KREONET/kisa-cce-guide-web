...
sourceAnnotations:
  - annotationIdentifier: u-26-source-001
    annotationType: sourceInconsistency
    targetType: astNode
    targetReference: "u-26:overview.inspectionContent.paragraph:1"
    sourceLocation:
      physicalPage: 53
      printedPage: "53"
      pageRegionIdentifier: p53-u-26
    sourceText: "허용할 호스트에 대한 접속 IP주소 제한 및 포트 제한 설정 여부 점검"
    explanation: "점검 내용과 점검 목적이 기준 제목 및 나머지 내용과 다른 주제를 설명한다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 53의 기준 제목, 점검 내용, 점검 목적, 보안 위협 및 조치 사례를 대조했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
...
> **참고**
>
> **`/dev` 디렉터리:** 논리적 장치 파일을 담고 있는 디렉터리이며 `/devices` 디렉터리에 있는 물리적 장치 파일에 대한 심볼릭 링크임. 예를 들어 `rmt0`를 `rmto`로 잘못 입력한 경우, `rmto` 파일이 새로 생성되는 것과 같이 디바이스 이름 입력 오류 시 root 파일 시스템이 에러를 일으킬 때까지 `/dev` 디렉터리에 계속해서 파일을 생성함
