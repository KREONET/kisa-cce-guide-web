...
classification:
  ...
  categoryIdentifier: web-application-insufficient-session-management
...
1. 로그인 과정에서 발급받은 세션 ID를 확인하고 로그아웃 후 재 로그인 시 각각 발급받은 세션 ID에 대해 일정한 패턴이 존재하는지 검증
   - 각각 발급받은 세션 ID를 확인하여 예측 가능한 패턴으로 생성되는지 검증
   - 각각 발급받은 세션 ID를 확인하여 고정된 값으로 생성되는지 검증
2. 로그인 후 웹 페이지 사용을 중지한 상태로 일정 시간이 경과하였을 때 세션이 유지되는지 확인

**JWT(JSON Web Token)**

1. 사용자 인증 및 상태 유지를 위해 JWT(JSON Web Token)가 사용되는지 확인

   JWT는 일반적으로 `Authorization` 헤더에 포함되고, 경우에 따라 `Cookie` 헤더에 포함, 또는 `POST` 데이터로 전송할 수 있음

2. JWT(JSON Web Token)의 헤더와 페이로드 부분을 각각 Base64 디코딩하여 내용을 확인할 수 있으며, 안전한 알고리즘을 사용하는지, 토큰 만료 설정이 적절하게 되었는지, 민감정보가 노출되는지 등을 확인
3. 페이로드 변조 후 서명을 삭제하거나 None 알고리즘으로 변조하였을 때 토큰이 유효하여 타 계정의 권한 탈취가 가능한지 확인
...
1. 신규 로그인 시 세션 ID를 변경하고, Spring Security 설정에서 세션 고정을 방지하며 세션 ID를 생성
...
1. `web.config` 파일 설정을 통해 불규칙 세션 ID 발급 설정
...
1. 기존 세션을 삭제하고 새로운 세션 ID를 생성하며, 예측 불가한 안전한 세션 ID를 생성
...
1. `web.xml` 파일 설정을 통해 세션 만료 시간을 설정
...
2. Spring Boot의 경우 `application.properties`를 통해 세션 만료를 제어

   ```properties configuration
   server.servlet.session.timeout=60m
   ```

3. Spring Boot의 경우 소스코드를 통해 세션 만료를 제어
...
1. `web.config` 파일 설정을 통해 세션 만료 시간을 설정
...
2. `Global.asax` 파일에서 세션을 확인하고 접근을 제어하는 로직을 추가
...
1. `php.ini` 파일 설정을 통해 세션 만료 시간을 설정
...
2. `.htaccess` 파일 설정을 통해 세션 만료 시간을 설정
...
4. 안전한 서명 알고리즘을 사용하더라도 약한 키(짧거나 예측 가능한 키)는 추측될 위험이 있으므로, 강력하고 랜덤한 비밀 키를 사용하고, 유출되지 않도록 키를 안전하게 관리
5. 토큰이 재사용되지 않도록 짧은 만료 시간을 설정하고, 리프레시 토큰을 사용하여 주기적으로 토큰을 갱신

| 구분 | 알고리즘 | 설명 |
| --- | --- | --- |
| 안전한 JWT 서명 알고리즘 | HS256~512 | 비밀 키를 사용하여 메시지에 해시를 적용 (숫자가 높을수록 긴 출력 길이를 가지며, 높은 수준의 보안을 제공) |
| 안전한 JWT 서명 알고리즘 | RS256~512 | 비대칭 키 쌍을 사용하여 서명 생성. 서명은 개인 키로 생성되고 공개 키로 검증 |
| 안전한 JWT 서명 알고리즘 | ES256~512 | 타원 곡선 암호화를 사용하여 서명을 생성 |
| 취약한 JWT 서명 알고리즘 | HS1 | 취약한 암호화 기술인 SHA-1을 기반으로 함 |
| 취약한 JWT 서명 알고리즘 | RS1 | 취약한 암호화 기술인 SHA-1을 기반으로 함 |
| 취약한 JWT 서명 알고리즘 | none | 서명을 생략하여 무결성 검증에 취약함 |
| 취약한 JWT 서명 알고리즘 | plaintext | 서명을 평문으로 전달하여, 무결성 검증에 취약함 |
