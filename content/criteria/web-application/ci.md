...
sourceAnnotations:
  - annotationIdentifier: ci-source-001
    annotationType: sourceInconsistency
    targetType: astNode
    targetReference: "ci:remediation.linux.configuration:11"
    sourceLocation:
      physicalPage: 692
      printedPage: "692"
      pageRegionIdentifier: p692-ci
    sourceText: "$userInput = htmlspecialchars($userInput ENT_QUOTES, 'UTF-8');"
    explanation: "원문 코드에서 첫 번째 인수와 ENT_QUOTES 사이의 쉼표가 보이지 않는다. 원문을 그대로 보존했다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 692의 PHP 예시를 확인했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
...
> **참고**
>
> **LDAP 인젝션:** 입력값이 LDAP(Lightweight Directory Access Protocol) 쿼리에 삽입되어 디렉터리 서비스 데이터에 대한 비인가된 조회, 수정, 삭제를 유발하는 공격
...
1. 사용자가 입력 가능한 파라미터 값에 다음 지시어를 삽입하여 전송 후 반환되는 페이지에 사이트의 홈 디렉터리가 표시되는지 확인

   ```html literal
   <!--#echo var="DOCUMENT_ROOT" -->
   ```

2. 사용자가 입력 가능한 파라미터 값에 다음 지시어를 삽입하여 전송 후 반환되는 페이지에 디렉터리의 파일 리스트가 표시되는지 확인
...
1. 취약점 존재 유무 판단을 위한 Xpath 쿼리 삽입. 예시로 제시한 것으로, 웹 사이트 환경에 맞춰 점검해야 함
...
2. 추가적인 쿼리 질의를 통하여 데이터 추출 등의 타당성 검토
...
1. 파일 업로드 페이지, API 엔드포인트, HTML Form 등 XML을 파싱하는 페이지 포인트 내 XML 객체 삽입 시도. 예로 제시한 것으로, 웹 사이트 환경에 맞춰 점검해야 함
...
2. 상위 컨텍스트 및 객체 접근을 시도하여 원격 코드 실행의 가능 여부를 판별하기 위하여 페이로드 입력 및 실행 유무 확인
