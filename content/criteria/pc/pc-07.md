...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 571
      physicalPageEnd: 572
      printedPageStart: "571"
      printedPageEnd: "572"

sourceAnnotations:
  - annotationIdentifier: pc-07-source-001
    annotationType: sourceInconsistency
    targetType: astNode
    targetReference: "pc-07:assessment.judgment.vulnerable:1"
    sourceLocation:
      physicalPage: 571
      printedPage: "571"
      pageRegionIdentifier: p571-pc-07
    sourceText: "모든 디스크 볼륨의 파일 시스템이 FAT32인 경우"
    explanation: "판단 기준은 모든 볼륨이 FAT32인 경우로 한정하지만, Step 2는 NTFS가 아닌 볼륨이 있는 경우 취약점이 존재한다고 설명한다."
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - "PDF physical page 571의 판단 기준과 Step 2를 대조했다."
    reviewedBy: null
    reviewedAt: null
    approvedBy: null
    approvedAt: null
...
1. 디스크 볼륨의 파일 시스템이 “NTFS”인지 확인

   - 시작 > 모든 앱 > Windows Tools > 컴퓨터 관리 > 저장소 > 디스크 관리
   - 시작 > 모든 앱 > Windows Tools > 실행 > “`diskmgmt.msc`” 입력 > 디스크 관리

2. 모든 디스크 볼륨의 파일 시스템이 “NTFS”가 아닌 경우 취약점이 존재하므로, 모든 디스크 볼륨에 대해 파일 시스템을 “NTFS”로 변경

3. NTFS 변경 후 폴더 및 파일에 적합한 ACL 적용
