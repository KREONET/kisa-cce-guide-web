...
criterion:
  code: C-06
  slug: c-06
  title: 제어시스템 운전원별 유일 계정 부여 또는 시간별 사용자 기록 유지
  severity:
    level: medium
    sourceLabel: 중
...
targetIdentifiers:
  ...
  - aix
  - hp-ux
sourceTargetText: "운전원이 접근하는 모든 제어시스템 구성요소"

provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 482
      physicalPageEnd: 483
      printedPageStart: "482"
      printedPageEnd: "483"
...
> **참고**
>
> **책임 추적성(Accountability):** 정보시스템의 접속자, 접속시간, 접속 위치, 업무내역 등을 식별하여 장애 원인, 침해사고 경위 등을 조사하는 과정에서 책임을 규명할 수 있도록 하는 것을 의미
...
2. 제어시스템 내 정보시스템의 운영체제 계정(ID) 목록과 각 계정을 사용하는 인원의 명단을 확인(이 경우 다음 사항을 포함하여 점검)
   ...
   - **Windows OS:** 운영자, 관리자별 계정 발급 확인

     ```text literal
     시작 > 제어판 > 사용자 계정
     ```

   - **Windows OS:** (개선조치 시) 제어시스템 운영자, 관리자별 계정 추가

     ```text literal
     시작 > 제어판 > 사용자 계정 > 추가
     ```

   - **Unix OS:** 운영자, 관리자별 계정 발급 확인

     ```shell command
     #cat /etc/passwd
     ```

   - **Unix OS:** (개선조치 시) 제어시스템 운영자, 관리자별 계정 추가
...
4. 사용기록은 사용자의 신원을 식별할 수 있는 방안을 포함하는지 확인(예시: 근무자 표 작성 및 근무자 접근 단말 기록 등)
