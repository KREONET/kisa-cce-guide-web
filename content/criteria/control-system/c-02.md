...
criterion:
  code: C-02
  slug: c-02
  title: 제어시스템 계정의 로그인/로그아웃, 사용 명령 등 사용기록을 저장
  severity:
    level: high
    sourceLabel: 상
...
targetIdentifiers:
  - linux
sourceTargetText: "공통, Windows OS, Unix OS"

provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 475
      physicalPageEnd: 476
      printedPageStart: "475"
      printedPageEnd: "476"
...
> **참고**
>
> **접속기록(Log):** 시스템(운영체제) 또는 응용 프로그램의 접속기록은 접속자(예시: ID), 접속일시, 접속자의 위치(예시: IP주소), 사용내역(예시: 운영체제의 명령 실행 또는 제어설비에 밸브 조작 명령 실행) 등 4가지 유형의 정보를 포함하는 데이터
...
1. 로그인/로그아웃, 사용 명령 등의 사용기록이 저장되도록 설정되어 있는지 확인하고, 생성된 사용기록(별도 파일 또는 DB)의 내용에 사용자의 로그인, 로그아웃, 사용 명령의 내용을 포함하고 있는지 확인

2. **Windows OS:** 로깅 설정 및 로그 파일의 로깅 정보 확인
...
3. **Unix OS:** `#cat /etc/syslog.conf` 파일에서 로그 설정을 확인하고, `#cat /var/adm/loginlog`, `authlog`, `sulog`의 파일을 열어 로깅 정보 확인

   ```shell command
   #cat /etc/syslog.conf
   ```

4. **Unix OS:** `#vi /etc/syslog.conf` 파일 설정 변경

   ```shell command
   #vi /etc/syslog.conf
   ```

5. **Unix OS:** Unix 계열 운영체제의 생성 로그 종류 확인
