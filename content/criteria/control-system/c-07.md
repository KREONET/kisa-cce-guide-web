...
targetIdentifiers:
  ...
  - aix
  - hp-ux
sourceTargetText: "모든 제어시스템 구성요소"
...
> **참고**
>
> 제어 H/W 등의 일부 구성요소에서 시간 동기화 기능이 `NTP` 또는 `PTP`와 같은 프로토콜을 이용하지 않으나 상시 연결되어있는 `EWS` 또는 `OWS` 등의 다른 구성요소와 시간 동기화를 수행하고 있고, 해당 구성요소가 운영체제 등을 통해 정상적으로 시간 동기화를 수행하고 있는 경우 정상적으로 시간 동기화를 수행하는 것으로 볼 수 있음
...
1. 제어시스템 구성요소에서 시각 동기화 기능을 제공하고 있는지 확인
   ...
   제어 H/W, 제어 S/W, 네트워크 장비, 정보시스템의 `NTP` 서버 설정 화면을 확인

> **참고**
>
> Cisco 스위치의 경우 다음 명령어로 확인 가능
...
> **참고**
>
> Microsoft Windows 제품의 경우 명령 프롬프트에서 다음 명령어로 확인 가능
