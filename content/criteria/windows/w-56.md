...
`로그인 시간이 만료되면 클라이언트 연결 끊기` 정책 `사용` 설정 및 `세션 연결을 중단하기 전에 필요한 유휴 시간` 정책 `15분` 이하로 설정
...
2. `로그인 시간이 만료되면 클라이언트 연결 끊기` 정책 `사용` 설정 및 `세션 연결을 중단하기 전에 필요한 유휴 시간` 정책 `15분` 이하로 설정

**Windows 2003, 2008, 2012, 2016, 2019, 2022**

1. 시작 > 제어판 > 관리 도구 > 로컬 보안 정책 > 로컬 정책 > 보안 옵션
2. `Microsoft 네트워크 서버: 로그온 시간이 만료되면 클라이언트 연결 끊기` 정책 `사용` 설정 및 `Microsoft 네트워크 서버: 세션 연결을 중단하기 전에 필요한 유휴 시간` 정책 `15분` 이하로 설정
