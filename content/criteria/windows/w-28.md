...
1. 다음 명령으로 레지스트리 편집기 실행

   ```text command
   시작 > 실행 > regedit
   ```

2. 다음 레지스트리 경로의 `MinEncryptionLevel` 값을 `2(중간)` 이상으로 설정
...
1. 다음 절차로 “해당 서비스”의 속성 열기

   ```text command
   시작 > 실행 > TSCC.MSC > “해당 서비스” 선택 > 속성
   ```

2. 암호화 수준을 `중간(Windows 2000)` 이상으로 설정
...
1. 다음 절차로 “해당 서비스”의 속성 열기
   ...
   ```text command
   Windows 2003: 시작 > 실행 > TSCC.MSC > “해당 서비스” 선택 > 속성
   ```
...
2. 다음 정책 경로로 이동
   ...
   ```text configuration
   컴퓨터 구성 > 관리 템플릿 > 터미널 서비스 > 원격 데스크톱 세션 호스트 > 보안
   ```

3. `[클라이언트 연결 암호화 수준 설정]`에서 `[암호화 수준]`을 `클라이언트 호환 가능`으로 설정
