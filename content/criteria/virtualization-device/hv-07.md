...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 809
      physicalPageEnd: 811
      printedPageStart: "809"
      printedPageEnd: "811"
...
4. `5` 이하로 설정되어 있지 않은 경우 [옵션 편집]을 클릭하여 아래와 같이 수정

5. `Security.AccountUnlockTime` 설정이 `600초(10분)` 이상으로 설정되어 있는지 확인

6. `600초(10분)` 이상으로 설정되어 있지 않은 경우 [옵션 편집]을 클릭하여 아래와 같이 수정
...
1. 예시) RHEL 8 이후 버전 기반 리눅스 아래 경로 설정 파일 확인
