---
schemaVersion: 1
contentModel: extractedCriterion
contentModelVersion: 1
criterion:
  code: PC-14
  slug: pc-14
  title: 바이러스 백신 프로그램에서 제공하는 실시간 감시 기능 활성화
  severity:
    level: high
    sourceLabel: 상
classification:
  domainIdentifier: pc
  categoryIdentifier: pc-security-management
targetScope: nonExhaustive
targetIdentifiers:
  - unspecified
sourceTargetText: 자동 전사본에서 확인 필요
provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 584
      physicalPageEnd: 584
      printedPageStart: '584'
      printedPageEnd: '584'
sourceAnnotations: []
---

## 원문 전사

### PDF 페이지 584

~~~text transcription
개요
점검 내용 시스템에 설치된 백신 프로그램의 환경 설정에 실시간 감시기능 적용 여부 점검
사용자가 인터넷(이동식 저장 매체 포함)을 통해 파일을 다운로드하거나 다운로드 받은 파일을 실행할
점검 목적
경우 백신 프로그램이 악성코드 감염을 실시간으로 점검하고 있는지 확인하기 위함
백신 프로그램의 실시간 감시기능이 적용되어 있지 않을 경우, 악성코드에 대해 실시간 감지가
보안 위협 이루어지지 않아 시스템 사용자가 인터넷(이동식 저장 매체 포함)을 통한 파일 다운로드나 실행 시
악성코드가 감염될 위험이 존재함
참고 -
점검 대상 및 판단 기준
대상 Windows 10, Windows 11
양호 : 설치된 백신의 실시간 감시기능이 활성화된 경우
판단 기준
취약 : 백신이 설치되어 있지 않거나 실시간 감시기능이 비활성화된 경우
조치 방법 바이러스 백신 실시간 감시 기능 설정
조치 시 영향 일반적인 경우 영향 없음
점검 및 조치 사례
l Windows 10, Windows 11
Step 1) 실시간 보호 기능 활성화
시작 > 모든 앱 > Windows 보안 > 바이러스 및 위협 방지 > 설정 관리 > 실시간 보호 기능 활성화
[ 실시간 보호 기능 활성화 ]
※ 상용 백신도 동일하게 실시간 보호 기능 활성화 적용
~~~

![PDF 페이지 584의 원문 점검항목 영역](../assets/pc-14/pc-14-page-584-source-region.png)
