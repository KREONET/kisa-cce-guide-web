...
사용하지 않는 서비스나 기본으로 설치된 서비스들을 제거하여 시스템 자원의 낭비를 막고 해당 서비스 포트를 통한 침입을 방지하기 위함

### 보안 위협

실질적 사용하지 않는 서비스들이 실행되어 시스템에 과부하가 발생하는 위험이 존재하며, 불필요한 서비스의 경우 사용자가 알지도 못한 서비스들이 실행되고 있는 경우가 대부분으로 해당 서비스가 이미 취약한 버전의 서비스인지도 인지하지 못하고 사용하는 위험이 존재함
...
> **참고**
>
> 시스템 관리자는 대상 시스템의 용도를 정확히 파악한 후 특별한 목적으로 사용하는 업무 관련 서비스를 제외한 다른 불필요한 서비스를 제거해야 함
...
1. 시작 > 모든 앱 Windows Tools > 서비스 > 해당 서비스 선택 > 속성으로 이동

   시작 > 모든 앱 > Windows Tools > 실행에서 `services.msc` 입력 > 해당 서비스 선택 > 속성으로도 설정 가능

2. 불필요한 서비스는 중지하고, `시작 유형`은 `사용 안 함`으로 설정
...
| DHCP Client | IP주소와 DNS 이름을 DHCP 서버에 등록하거나 DHCP 서버로부터 동적으로 IP주소를 가져오는 기능을 수행. 단독으로 시스템을 수행하며 고정IP를 사용하는 경우 불필요한 서비스 |
...
| Portable Media Serial Number | 컴퓨터에 연결된 이동성 미디어의 등록번호를 가져오는 기능 |
