...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 566
      physicalPageEnd: 569
      printedPageStart: '566'
      printedPageEnd: '569'
sourceAnnotations: []
---
...
~~~text transcription
개요
점검 내용 사용하지 않는 서비스나 기본으로 설치되어 실행되고 있는 서비스 여부 점검
사용하지 않는 서비스나 기본으로 설치된 서비스들을 제거하여 시스템 자원의 낭비를 막고 해당 서비스
점검 목적
포트를 통한 침입을 방지하기 위함
Ÿ 실질적 사용하지 않는 서비스들이 실행되어 시스템에 과부하가 발생하는 위험이 존재함
보안 위협 Ÿ 불필요한 서비스의 경우 사용자가 알지도 못한 서비스들이 실행되고 있는 경우가 대부분, 이 경우
...
![PDF 페이지 566의 원문 점검항목 영역](../assets/pc-05/pc-05-page-566-source-region.png)
...
![PDF 페이지 567의 원문 점검항목 영역](../assets/pc-05/pc-05-page-567-source-region.png)
...
DHCP Client 가져오는 기능을 수행. 단독으로 시스템을 수행하며 고정IP를 사용하는 경우 불필요한
서비스
Distributed Link Tracking 네트워크 도메인의 여러 컴퓨터나 일반 컴퓨터에서 NTFS 파일간의 연결을 관리하는 도구.
Client, Server Active Directory가 구성되어 있지 않은 서버에서는 불필요한 서비스
...
Service
Human Interface 키보드 또는 기타 멀티미디어 장치에 사전 정의된 버튼들을 사용하는 HID 장치들을 위한
Device Access 서비스
IMAPI CD-Burning 서버에 CD-RW 또는 DVD-RW가 장착되어 보조백업장치 역할을 하기 위해서 자체
COM Service 레코딩 백업을 할 수 있음
Infrared Monitor 사용자 적외선 연결을 통해 파일 및 이미지를 공유할 수 있도록 함
Messenger 클라이언트와 서버 사이에 netsend 및 경고서비스 메시지를 전송하는 기능
NetMeeting Remote 윈도우9X 운영체제부터 인증된 사용자가 넷미팅을 사용해서 원격으로 컴퓨터에 접근할 수
Desktop Sharing 있도록 하는 기능
...
Remote Registry 원격 사용자가 컴퓨터에서 레지스트리 설정을 수정할 수 있도록 설정하는 응용 프로그램
Simple TCP/IP
...
![PDF 페이지 568의 원문 점검항목 영역](../assets/pc-05/pc-05-page-568-source-region.png)
...
![PDF 페이지 569의 원문 점검항목 영역](../assets/pc-05/pc-05-page-569-source-region.png)
