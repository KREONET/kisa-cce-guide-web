...
classification:
  ...
  categoryIdentifier: web-application-directory-indexing
...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 700
      physicalPageEnd: 702
      printedPageStart: "700"
      printedPageEnd: "702"
...
> **참고**
>
> **디렉터리 인덱싱 취약점:** 특정 디렉터리의 초기 페이지(`index.html`, `home.html`, `default.asp` 등) 파일이 존재하지 않을 때 디렉터리 목록을 출력하는 취약점
...
2. **Apache:** `httpd.conf` 파일 내 `Indexes` 옵션 제거 후 서버 재기동
   ...
   ```
3. **Tomcat:** `web.xml` 파일 내 아래 지시자 수정 후 서버 재기동
   ...
   ```
4. **IIS (6.0 이하):** 인터넷 정보 서비스 → 등록 정보 → 홈 디렉터리 → 디렉터리 검색 해제
5. **IIS (7.0 이상):** IIS 관리자 → 디렉터리 검색 → 사용 안 함
6. **Nginx:** `/etc/nginx/sites-available/default` 등 파일 내 아래 지시자 수정 후 서버 재기동
