---
schemaVersion: 1
contentModel: extractedCriterion
contentModelVersion: 1
criterion:
  code: W-37
  slug: w-37
  title: 예약된 작업에 의심스러운 명령이 등록되어 있는지 점검
  severity:
    level: medium
    sourceLabel: 중
classification:
  domainIdentifier: windows
  categoryIdentifier: windows-service-management
targetScope: nonExhaustive
targetIdentifiers:
  - unspecified
sourceTargetText: 자동 전사본에서 확인 필요
provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 232
      physicalPageEnd: 232
      printedPageStart: '232'
      printedPageEnd: '232'
sourceAnnotations: []
---

## 원문 전사

### PDF 페이지 232

~~~text transcription
개요
점검 내용 예약된 작업에 의심스러운 명령의 등록 여부 점검
점검 목적 외부 무단 침입 시 설정될 수 있는 불필요한 예약 작업의 등록 여부를 확인하기 위함
일정 시간마다 미리 설정해둔 프로그램을 실행할 수 있는 예약된 작업은 시작 프로그램과 더불어서
보안 위협
해킹과 트로이목마, 백도어를 설치하여 공격하기 좋은 경로로 사용될 위험이 존재함
참고 -
점검 대상 및 판단 기준
대상 Windows 2000, 2003, 2008, 2012, 2016, 2019, 2022
양호 : 불필요한 명령어나 파일 등 주기적인 예약 작업의 존재 여부를 주기적으로 점검하고 제거한 경우
판단 기준 취약 : 불필요한 명령어나 파일 등 주기적인 예약 작업의 존재 여부를 주기적으로 점검하지 않거나, 불필
요한 작업을 제거하지 않은 경우
조치 방법 예약 작업에 대한 주기적인 확인
조치 시 영향 예약 작업을 잘못 삭제하는 경우 관련된 작업이 실행되지 않을 수 있음
점검 및 조치 사례
l Windows 2000, 2003, 2008, 2012, 2016, 2019, 2022
Step 1) 시작 > 설정 > 제어판 > 예약된 작업 확인
Step 2) 등록된 예약 작업을 선택하여 상세 내역 확인
Step 3) 불필요한 작업 존재 시 삭제
[ 불필요한 작업 제거 ]
※ 2008, 2012, 2016, 2019 는 제어판 > 관리 도구 > 작업 스케줄러에서 확인
~~~

![PDF 페이지 232의 원문 점검항목 영역](../assets/w-37/w-37-page-232-source-region.png)
