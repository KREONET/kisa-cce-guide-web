...
점검 목적 일반적으로 로그인이 필요치 않은 해당 계정들을 통한 로그인을 차단하고, 계정의 패스워드 추측 공격
시도를 차단하기 위함
관리되지 않은 불필요한 계정은 장기간 비밀번호가 변경되지 않아 무차별 대입 공격(Brute Force
보안 위협 Attack)이나 비밀번호 추측 공격(Password Guessing Attack)의 가능성이 존재하며, 또한 이런
...
판단 기준
취약 : 불필요한 계정이 존재하는 경우
조치 방법 현재 계정 현황 확인 후 불필요한 계정 삭제
...
![PDF 페이지 179의 원문 점검항목 영역](../assets/w-03/w-03-page-179-source-region.png)
