---
schemaVersion: 1
contentModel: extractedCriterion
contentModelVersion: 1
criterion:
  code: W-07
  slug: w-07
  title: Everyone 사용 권한을 익명 사용자에 적용
  severity:
    level: medium
    sourceLabel: 중
classification:
  domainIdentifier: windows
  categoryIdentifier: windows-account-management
targetScope: nonExhaustive
targetIdentifiers:
  - unspecified
sourceTargetText: 자동 전사본에서 확인 필요
provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 184
      physicalPageEnd: 184
      printedPageStart: '184'
      printedPageEnd: '184'
sourceAnnotations:
  - annotationIdentifier: w-07-inventory-001
    annotationType: sourceInconsistency
    targetType: metadata
    targetReference: /criterion/title
    sourceLocation:
      physicalPage: 184
      printedPage: '184'
      pageRegionIdentifier: p184-w-07
    sourceText: Everyone 사용 권한을 익명 사용자에 적용
    explanation: '{"anomalyType": "detailHeaderTitleMismatch", "code": "W-07", "detailHeaderTitle": "Everyone 사용 권한을 익명 사용자에 적용", "physicalPage": 184, "resolution": "Use the detailed criterion heading as the canonical title and preserve the chapter summary value as an annotation.", "summaryTitle": "Everyone 사용 권한을 익명 사용자에게 적용"}'
    disposition: unresolved
    reviewStatus: pending
    verificationEvidence:
      - Use the detailed criterion heading as the canonical title and preserve the chapter summary value as an annotation.
    reviewedBy:
    reviewedAt:
    approvedBy:
    approvedAt:
---

## 원문 전사

### PDF 페이지 184

~~~text transcription
개요
점검 내용 “Everyone 사용 권한을 익명 사용자에 적용” 정책의 설정 여부 점검
익명 사용자가 Everyone 그룹으로 사용 권한을 준 모든 리소스에 접근하는 것을 차단하여 비인가자에
점검 목적
의한 접근 가능성을 제한하기 위함
해당 정책이 “사용”으로 설정될 경우 권한이 없는 사용자가 익명으로 계정 이름 및 공유 리소스를
보안 위협 나열하고 이 정보를 사용하여 암호를 추측하거나 DoS(Denial of Service) 공격을 실행할 위험이
존재함
※ DoS(Denial of Service): 관리자 권한 없이도 특정 서버에 처리할 수 없을 정도로 대량의 접속 신
참고
호를 한꺼번에 보내 해당 서버가 마비되도록 하는 해킹 기법
점검 대상 및 판단 기준
대상 2003, 2008, 2012, 2016, 2019, 2022
양호 : “Everyone 사용 권한을 익명 사용자에게 적용” 정책이 “사용 안 함”으로 되어 있는 경우
판단 기준
취약 : “Everyone 사용 권한을 익명 사용자에게 적용” 정책이 “사용”으로 되어 있는 경우
조치 방법 “Everyone 사용 권한을 익명 사용자에게 적용”정책을 “사용 안 함”으로 설정
조치 시 영향 응용프로그램이나 Backup 용도로 Everyone 공유를 사용하지 않는지 확인 필요
점검 및 조치 사례
l Windows 2003, 2008, 2012, 2016, 2019, 2022
Step 1) 시작 > 제어판 > 관리 도구 > 로컬 보안 정책 > 로컬 정책 > 보안 옵션
Step 2) “Everyone 사용 권한을 익명 사용자에게 적용”을 “사용 안 함”으로 설정
[ Everyone 사용 권한을 익명 사용자에게 적용 정책 설정 ]
~~~

![PDF 페이지 184의 원문 점검항목 영역](../assets/w-07/w-07-page-184-source-region.png)
