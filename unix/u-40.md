...
NFS 서비스를 사용하지 않는 경우 서비스 중지 및 비활성화 설정, 불가피하게 사용 시 접근 통제 설정 및 NFS 설정 파일 접근 권한 644 설정
...
5. `/etc/dfs/dfstab` 파일에 디렉터리 공유를 허용할 사용자 및 해당 사용자의 권한 설정
   ...
   ```shell configuration
   share -F nfs -o rw, ro /export/home/example
...
5. `/etc/exports` 파일에 디렉터리 공유를 허용할 사용자 및 해당 사용자의 권한 설정
   ...
   ```text configuration
   /home/example        host1 (ro, root_squash)
   ```
...
5. `/etc/exports` 파일에 디렉터리 공유를 허용할 사용자 및 해당 사용자의 권한 설정
   ...
   ```text configuration
   /home/example –sec=sys:krb5p:krb5i:krb5:dh,ro=host1, access=host1
   ```
