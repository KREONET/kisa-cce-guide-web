...
criterion:
  code: C-13
  slug: c-13
  title: 제어시스템 내 전달되는 제어 명령 및 파라미터의 정상 범위를 식별하고 관리
  severity:
    level: low
    sourceLabel: 하
...
targetIdentifiers:
  ...
  - aix
  - hp-ux
sourceTargetText: "HMI, PLC 등의 제어 S/W"

provenance:
  sourceDocumentIdentifier: kisa-cce-criteria-2026
  sourcePageRanges:
    - physicalPageStart: 497
      physicalPageEnd: 497
      printedPageStart: "497"
      printedPageEnd: "497"
...
정상 범위를 초과하는 제어 명령 값을 부여할 수 있다면 부주의(Human Error), 오·남용이 발생할 수 있고 특히 악성코드에 의한 악의적인 운전 명령 및 과도한 파라미터 값 전달을 통해 제어설비의 오작동 등의 피해가 발생할 수 있음. 위협 사례로, 스턱스넷은 PLC 시스템의 Profibus 메시지 버스 시스템을 감시하는 `D8890` 블록에 악성코드를 설치하여 특정 조건을 만족하면, 주기적으로 모터의 회전수를 `1410Hz`, `2Hz`, `1064Hz`로 변경해 모터에 과부하를 일으킴. 또한 PLC 시스템에 루트킷을 설치하여 자기 자신을 숨기고, 모터의 회전수가 변경되고 있다는 것을 숨김
