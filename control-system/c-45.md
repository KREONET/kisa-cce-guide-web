...
criterion:
  code: C-45
  slug: c-45
  title: 제어시스템에서 USB 등 이동형 저장 매체를 사용해야 하는 경우, 사전 정의된 정책에 따라 사용
  severity:
    level: high
    sourceLabel: 상
classification:
...
provenance:
  ...
  sourcePageRanges:
    - physicalPageStart: 542
      physicalPageEnd: 543
      printedPageStart: '542'
      printedPageEnd: '543'
sourceAnnotations: []
---
...
참고 - 이동형 저장 매체의 사용이 불가피한 경우, 사전에 인가된 저장 매체를 활용하고 해당 저장 매체의
사용 승인절차와 안전조치(예시: 사용 전, 사용 후 악성코드 감염 여부 검사)가 적용되도록 하는
통제절차 필요
점검 대상 및 판단 기준
대상 제어시스템을 구성하는 서버, PC 등 이동형 저장 매체가 연결 가능한 시스템
양호 : 이동형 저장 매체 통제 정책을 수립하고 이동형 저장 매체 사용 시 수립된 정책에 따르는 경우
판단 기준 취약 : 이동형 저장 매체 통제 정책을 수립하지 않았거나 이동형 저장 매체 사용 시 수립된 정책에 따르지
않는 경우
조치 방법 이동형 저장 매체에 대한 사용통제 정책을 수립하고 불가피한 경우 정책에 따라 사용하도록 조치
조치 시 영향 일반적인 경우 영향 없음
~~~

![PDF 페이지 542의 원문 점검항목 영역](../assets/c-45/c-45-page-542-source-region.png)
...
![PDF 페이지 543의 원문 점검항목 영역](../assets/c-45/c-45-page-543-source-region.png)
